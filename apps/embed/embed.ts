...
(function () {
  ...
  if (!organizationId) {
    console.error("Loco Widget: data-organization-id attribute is required");
    return;
  }
  ...
  function render() {
    ...
    button.style.cssText = `
      ...
      height: 60px;
      border-radius: 50%;
      background: ${EMBED_CONFIG.PRIMARY_COLOR};
      color: white;
      border: none;
      cursor: pointer;
      z-index: 999999;
      ...
      box-shadow: 0 4px 24px ${EMBED_CONFIG.PRIMARY_COLOR_SHADOW};
    ...
    container.id = "loco-widget-container";
  ...
  function hide() {
    if (container && button) {
      ...
      button.innerHTML = chatBubbleIcon;
      button.style.background = EMBED_CONFIG.PRIMARY_COLOR;
    }
  }
  ...
  (window as unknown as Record<string, unknown>).LocoWidget = {
    init: reinit,
    show,
    hide,
    destroy,
